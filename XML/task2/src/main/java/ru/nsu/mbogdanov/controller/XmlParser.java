...
public class XmlParser {
    ...
    private ArrayList<PersonInfo> normalize(ArrayList<PersonInfo> originalData, Integer peopleCount) {
        ...
        System.out.println("\nИщем спаусов");
        var fullInfoWithSpouse = extractAllSpouseData(fullInfoWithChildren);

        System.out.println("\nДаём всем гендеры");
        var fullInfoWithChildrenAndGender = extractAllGenderData(fullInfoWithSpouse);

        System.out.println("\nПроверяем гендеры");
        genderChecker(fullInfoWithChildrenAndGender);

        System.out.println("\nИщем родителей");
        var fullInfoWithParents = extractAllParentsData(fullInfoWithChildrenAndGender);

        System.out.println("\nИщем всем сиблингов");
        var fullInfoWithChildrenAndGenderAndSiblings = extractAllSiblingsData(fullInfoWithParents);
        System.out.println("\nПроверяем сиблингов");
        siblingsChecker(fullInfoWithChildrenAndGenderAndSiblings);

        System.out.println("\nРазбиваем сиблингов на братьев и сестёр");
        var fullInfoWithSiblingsGenders = extractAllSiblingsDataByGender(fullInfoWithChildrenAndGenderAndSiblings);
        return new ArrayList<>(fullInfoWithSiblingsGenders.values());
    }
    ...
    private HashMap<String, PersonInfo> extractAllParentsData(HashMap<String, PersonInfo> oneIdToOnePersonHashMap) {
        int motherCounter = 0;
        int fatherCounter = 0;
        for (var person : oneIdToOnePersonHashMap.values()) {
            if (person.getParentsId() != null) {
                for (var parents : person.getParentsId()) {
                    var parent = oneIdToOnePersonHashMap.get(parents);
                    if (parent != null) {
                        if (person.getMotherId() == null && parent.getGender().equals("F")) {
                            person.setMotherId(parents);
                            motherCounter++;
                        } else if (person.getFatherId() == null && parent.getGender().equals("M")) {
                            person.setFatherId(parents);
                            fatherCounter++;
                        }
                    }
                }
        ...
        System.out.println("Помогли найти маму столько раз: " + motherCounter);
        System.out.println("Помогли найти папу столько раз: " + fatherCounter);
        return oneIdToOnePersonHashMap;
    }

    private HashMap<String, PersonInfo> extractAllSiblingsDataByGender(HashMap<String, PersonInfo> oneIdToOnePersonHashMap) {
        int brotherCounter = 0;
        int sisterCounter = 0;
        for (var person : oneIdToOnePersonHashMap.values()) {
            if (person.getSiblingsId() != null) {
                for (var siblings : person.getSiblingsId()) {
                    var siblingsModel = oneIdToOnePersonHashMap.get(siblings);
                    if (siblingsModel != null) {
                        if (siblingsModel.getGender().equals("F") && !person.getSistersId().contains(siblings)) {
                            person.addSistersId(siblings);
                            sisterCounter++;
                        } else if (siblingsModel.getGender().equals("M") && !person.getBrothersId().contains(siblings)) {
                            person.addBrothersId(siblings);
                            brotherCounter++;
                        }
                    }
                }
        ...
        System.out.println("Помогли найти брата столько раз: " + brotherCounter);
        System.out.println("Помогли найти сестру столько раз: " + sisterCounter);
        return oneIdToOnePersonHashMap;
    }

    private HashMap<String, PersonInfo> extractAllSpouseData(HashMap<String, PersonInfo> oneIdToOnePersonHashMap) {
        int counter = 0;
        for (var person : oneIdToOnePersonHashMap.values()) {
            if (person.getSpouseId() == null) {
                if (person.getWifeId() != null) {
                    person.setSpouseId(person.getWifeId());
                    if (person.getId() != null) {
                        oneIdToOnePersonHashMap.get(person.getWifeId()).setHusbandId(person.getId());
                        oneIdToOnePersonHashMap.get(person.getSpouseId()).setSpouseId(person.getId());
                        oneIdToOnePersonHashMap.get(person.getWifeId()).setSpouseId(person.getId());
                        if (oneIdToOnePersonHashMap.get(person.getWifeId()).getWifeId() != null){
                            counter++;
                        }
                    }
                } else if (person.getHusbandId() != null) {
                    person.setSpouseId(person.getHusbandId());
                    if (person.getId() != null) {
                        oneIdToOnePersonHashMap.get(person.getHusbandId()).setWifeId(person.getId());
                        oneIdToOnePersonHashMap.get(person.getSpouseId()).setSpouseId(person.getId());
                        oneIdToOnePersonHashMap.get(person.getHusbandId()).setSpouseId(person.getId());
                        if (oneIdToOnePersonHashMap.get(person.getHusbandId()).getWifeId() != null){
                            counter++;
                        }
                    }
                }
        ...
        System.out.println("Помогли найти парочку: " + counter);
        return oneIdToOnePersonHashMap;
    }
